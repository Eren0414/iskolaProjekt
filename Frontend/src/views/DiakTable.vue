<template>
  <div>
    ...
    <table class="table table-striped">
      ...
      <tbody>
        <tr v-for="dataLine in collection" :key="dataLine.id" @click="onClickTr(dataLine.id)"
          ...
          <td>{{ dataLine.nev }}</td>
          <td>{{ dataLine.helyseg }}</td>
          <td>{{ dataLine.osztondij }}</td>
          <td>{{ dataLine.atlag }}</td>
        </tr>
      </tbody>
    </table>
    ...
    <Modal :title="title" :yes="yes" :no="no" :size="size" @yesEvent="yesEventHandler">
      ...
      <DiakForm v-if="state == 'Create' || state == 'Update'" :dataLine="dataLine"
...
export default {
  components: { DiakForm, OsztalyForm, OperationsCrud, Modal },
  mounted() {
    this.loadSports();
  ...
  methods: {
    loadSports() {
      ...
      axios.get("http://localhost:8000/api/diaks", {
        headers: {
        ...
        .then(response => {
          ...
          this.collection = response.data.data.map(diak => ({
            ...diak,
            osztalyNev: diak.osztaly ? diak.osztaly.nev : null, // Osztály neve
          }));
        })
        .catch(error => {
